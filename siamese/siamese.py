...
class Siamese(object):
    ...
    def __init__(self, model_path=None, **kwargs):
        ...
        print("model_path:", self._defaults["model_path"])
    ...
    def detect_image(self, image_1, image_2):
        ...
        with torch.no_grad():
            ...
            photo_1 = torch.from_numpy(
                np.expand_dims(np.transpose(photo_1, (2, 0, 1)), 0)
            ).type(torch.FloatTensor)
            photo_2 = torch.from_numpy(
                np.expand_dims(np.transpose(photo_2, (2, 0, 1)), 0)
            ).type(torch.FloatTensor)
        ...
        plt.text(
            -12, -12, "Similarity:%.3f" % output, ha="center", va="bottom", fontsize=11
        )
        plt.show()
        return output
